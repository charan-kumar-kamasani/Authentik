...
function ResultAuthentic({ data }: { data: any }) {
 ...
 {/* Review & Claim Reward Banner */}
        ...
        {showCouponReveal && awardedCoupon && (
          <div className="fixed inset-0 z-[200] bg-white flex flex-col font-sans overflow-x-hidden overflow-y-auto" style={{ animation: 'couponFadeIn 0.35s cubic-bezier(0.16, 1, 0.3, 1)' }}>
            
            {/* Top Header */}
            <div className="flex items-center justify-center relative py-4 bg-white sticky top-0 z-50">
              <button onClick={() => setShowCouponReveal(false)} className="absolute left-4 p-2 -ml-2 text-[#0F172A] hover:bg-slate-50 rounded-full transition-colors">
            ...
            <div className="flex-1 px-4 py-2 flex flex-col items-center">
              ...
              <div className="mb-5 flex flex-col items-center">
                <div className="relative w-[180px] h-[120px] flex items-center justify-center mb-3">
                  ...
                  <svg width="80" height="80" viewBox="0 0 100 100" fill="none" xmlns="http://www.w3.org/2000/svg" className="relative z-10 drop-shadow-xl mt-4 animate-bounce" style={{ animationDuration: '2s' }}>
                    {/* Blue Box Front */}
              ...
              <div className="w-full bg-[#0A34B8] rounded-[12px] flex shadow-[0_8px_20px_rgba(10,52,184,0.15)] mb-6 overflow-hidden">
                ...
                <div className="flex-1 p-5 pr-2 flex flex-col justify-center text-white relative">
                  ...
                  <h3 className="text-[36px] font-black leading-none mb-1">
                    {awardedCoupon.title || (awardedCoupon.discountText || 'OFFER')}
                  </h3>
                  <p className="text-[14px] font-medium text-white mb-2">on {companyName || 'Origin Nutrition'}</p>
                ...
                <div className="w-[130px] p-4 pr-5 pl-3 flex flex-col items-center justify-center">
                  ...
                  }} className="w-full bg-transparent border border-white/30 hover:bg-white/10 active:scale-95 text-white rounded-[6px] py-1.5 flex items-center justify-center gap-1.5 transition-all">
                    {couponCopied ? (
                      <Check className="w-3 h-3" />
                    ) : (
                      <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><rect x="9" y="9" width="13" height="13" rx="2" ry="2"></rect><path d="M5 15H4a2 2 0 0 1-2-2V4a2 2 0 0 1 2-2h9a2 2 0 0 1 2 2v1"></path></svg>
                    )}
                    <span className="text-[11px] font-medium tracking-wide">{couponCopied ? 'Copied' : 'Copy Code'}</span>
                  </button>
                </div>
              </div>
                ...
                <div className="flex flex-col gap-4">
                  {awardedCoupon.websiteLink && (
                    <div className="flex items-center gap-3 border-b border-[#F1F5F9] pb-4">
                      ...
                      <a href={awardedCoupon.websiteLink.startsWith('http') ? awardedCoupon.websiteLink : `https://${awardedCoupon.websiteLink}`} target="_blank" rel="noreferrer" className="text-[14px] font-bold text-[#2563EB] flex items-center gap-1.5 hover:underline truncate max-w-[150px]">
                        {new URL(awardedCoupon.websiteLink.startsWith('http') ? awardedCoupon.websiteLink : `https://${awardedCoupon.websiteLink}`).hostname.replace('www.', '')} <ExternalLink className="w-[14px] h-[14px]" />
                      </a>
                    </div>
                  )}
                    ...
                    <div className="flex-1">
                      ...
                      <div className="text-[12px] text-[#475569] leading-relaxed">
                        {awardedCoupon.description ? (
                          <div dangerouslySetInnerHTML={{ __html: awardedCoupon.description.replace(/\n/g, '<br/>') }} />
                        ) : (
                          <>
                            Use code {awardedCoupon.code} at checkout to unlock your savings.<br/>
              ...
              <button 
                onClick={() => {
                  if (awardedCoupon.websiteLink) {
                    window.open(awardedCoupon.websiteLink.startsWith('http') ? awardedCoupon.websiteLink : `https://${awardedCoupon.websiteLink}`, '_blank');
                  } else {
                    navigate('/rewards');
                  }
                }}
