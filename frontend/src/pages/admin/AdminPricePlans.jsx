...
const DEFAULT_CONFIG = {
  ...
  quarterlyMultiplier: 1.1,
  yearlyMultiplier: 1.0,
  monthlyLabel: "Monthly",
  quarterlyLabel: "Quarterly",
  yearlyLabel: "Yearly",
};

const emptyCyclePricing = () => ({
  pricePerQr: 0,
  validity: "-",
  saveText: "-",
});
...
export default function AdminPricePlans() {
  ...
  const getPlanPrice = (plan) => {
    ...
    if (cp && cp.pricePerQr !== undefined && cp.pricePerQr !== null)
      return cp.pricePerQr;
    ...
    if (plan) {
      ...
      setForm({
        ...
        features: plan.features.map((f) => ({
          featureId: f.featureId?._id || f.featureId,
          value: f.value,
        })),
      });
    } else {
  ...
  const setCF = (cyc, field, val) => {
    setForm((prev) => ({
      ...
      pricing: {
        ...prev.pricing,
        [cyc]: { ...prev.pricing[cyc], [field]: val },
      },
    }));
  };
  ...
  const setFV = (fid, val) => {
    setForm((prev) => {
      ...
      else arr.push({ featureId: fid, value: val });
  ...
  const savePlan = async (e) => {
    ...
    const token = localStorage.getItem("adminToken");
    const url = editId
      ? `${API_BASE_URL}/plans/plans/${editId}`
      : `${API_BASE_URL}/plans/plans`;
    ...
    return (
      <div className="flex items-center justify-center min-h-[500px]">
        <div className="flex flex-col items-center gap-3">
          ...
          <p className="text-slate-400 font-semibold text-sm animate-pulse">
      ...
      <div className="bg-white rounded-2xl p-6 shadow-sm border border-slate-200/60 flex flex-wrap gap-4 items-center justify-between">
        <div>
          ...
          <p className="text-slate-500 text-sm mt-0.5 font-medium">
            Configure pricing tiers, features &amp; billing cycles
          </p>
        </div>
        <div className="flex items-center gap-3 flex-wrap">
          <button
            onClick={() => setPreview(!preview)}
            className={`flex items-center gap-2 px-4 py-2.5 rounded-xl text-sm font-bold border-2 transition-all ${preview ? "bg-indigo-50 border-indigo-200 text-indigo-700" : "bg-white border-slate-200 text-slate-600 hover:border-slate-300"}`}
          >
          ...
          <button
            onClick={() => {
              setConfigForm(billingConfig);
              setConfigModal(true);
            }}
            className="flex items-center gap-2 px-4 py-2.5 bg-white border-2 border-slate-200 text-slate-700 rounded-xl text-sm font-bold hover:border-slate-300 transition-all"
          >
            <Sliders size={16} /> Billing Config
          </button>
          <button
            onClick={() => setFeatModal(true)}
            className="flex items-center gap-2 px-4 py-2.5 bg-white border-2 border-slate-200 text-slate-700 rounded-xl text-sm font-bold hover:border-slate-300 transition-all"
          >
          ...
          <button
            onClick={() => openPlan()}
            className="flex items-center gap-2 px-4 py-2.5 bg-blue-600 text-white rounded-xl text-sm font-bold shadow-lg shadow-blue-600/25 hover:bg-blue-700 transition-all active:scale-95"
          >
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5 gap-5">
          {plans.map((plan) => {
            ...
            return (
              <div
                key={plan._id}
                className={`relative bg-white rounded-2xl border-2 flex flex-col transition-all duration-300 hover:-translate-y-1 hover:shadow-lg ${
                  popular
                    ? "border-purple-300 shadow-xl shadow-purple-500/10 scale-[1.02] z-10"
              ...
              >
                ...
                <div
                  className={`bg-gradient-to-br ${grad(plan.name)} text-white p-6 text-center ${!popular ? "rounded-t-xl" : ""}`}
                >
                  {plan.isTrial && (
                    <div className="text-[10px] font-black bg-white/20 inline-block px-2 py-0.5 rounded mb-2 uppercase tracking-wider">
                      Trial Plan
                    </div>
                  )}
                ...
                <div className="p-5 flex-1 flex flex-col">
                  ...
                  <div className="space-y-2.5 flex-1">
                    {features.map((feat) => {
                      const pf = plan.features.find(
                        (f) => (f.featureId?._id || f.featureId) === feat._id,
                      );
                      const val = pf?.value;
                ...
                <tr>
                  ...
                  {plans.map((plan) => {
                    ...
                    return (
                      <th
                        key={plan._id}
                        className={`border-b border-slate-100 text-center min-w-[170px] relative group ${plan.isPopular ? "bg-purple-50/20" : ""}`}
                      >
                        {plan.isPopular && (
                          <div className="absolute top-0 inset-x-0 h-1 bg-gradient-to-r from-purple-500 to-violet-500 rounded-b-sm" />
                        )}
                        {plan.isPopular && (
                        ...
                        <div
                          className={`mx-2 mt-5 mb-2 p-4 rounded-xl bg-gradient-to-br ${grad(plan.name)} text-white shadow-md`}
                        >
                          {plan.isTrial && (
                            <div className="text-[9px] font-black bg-white/20 inline-block px-1.5 py-0.5 rounded mb-1 uppercase">
                              Trial
                            </div>
                          )}
              ...
              <tbody>
                ...
                <tr className="border-b border-slate-100/50 bg-emerald-50/20">
                  ...
                  {plans.map((plan) => {
                    ...
                    return (
                      <td
                        key={plan._id}
                        className={`px-5 py-3.5 text-center text-sm font-black text-emerald-700 ${plan.isPopular ? "bg-purple-50/10" : ""}`}
                      >
                        {v && v !== "-" ? `${v} days` : "-"}
                      </td>
                    );
                ...
                <tr className="border-b border-slate-100/50 bg-amber-50/20">
                  ...
                  {plans.map((plan) => {
                    ...
                    return (
                      <td
                        key={plan._id}
                        className={`px-5 py-3.5 text-center text-sm font-black text-amber-700 ${plan.isPopular ? "bg-purple-50/10" : ""}`}
                      >
                        {s && s !== "-" ? (
                          <>
                            {"₹"}
                ...
                {[
                  ...
                  <tr key={row.key} className="border-b border-slate-100/50">
                    <td className="px-5 py-3.5 border-r border-slate-100 bg-slate-50/40 text-sm font-bold text-slate-600 sticky left-0 backdrop-blur-sm">
                      {row.label}
                    </td>
                    {plans.map((plan) => (
                      <td
                        key={plan._id}
                        className={`px-5 py-3.5 text-center text-sm font-black text-slate-700 ${plan.isPopular ? "bg-purple-50/10" : ""}`}
                      >
                        {plan[row.key] || "-"}
                      </td>
                    ))}
                ...
                <tr>
                  <td
                    ...
                    className="h-2 bg-slate-50/60 border-y border-slate-100/50"
                  />
                </tr>
                ...
                {features.map((feat) => (
                  <tr
                    key={feat._id}
                    className="border-b border-slate-100/50 hover:bg-slate-50/30 transition-colors group/row"
                  >
                    <td className="px-5 py-3.5 border-r border-slate-100 bg-white text-sm font-semibold text-slate-600 sticky left-0 backdrop-blur-sm">
                      <div className="flex items-center justify-between">
                        ...
                        <button
                          onClick={() => delFeature(feat._id)}
                          className="opacity-0 group-hover/row:opacity-100 text-red-400 hover:text-red-600 p-1 rounded transition-all"
                        >
                          <Trash2 size={12} />
                        </button>
                      </div>
                    ...
                    {plans.map((plan) => {
                      ...
                      if (!pf || val === false || val === "false") {
                        cell = (
                          <div className="w-7 h-7 mx-auto rounded-full bg-red-50 flex items-center justify-center border border-red-100">
                            <X
                              size={14}
                              className="text-red-300"
                              strokeWidth={3}
                        ...
                        cell = (
                          <div className="w-7 h-7 mx-auto rounded-full bg-emerald-100 flex items-center justify-center border border-emerald-200">
                            <Check
                              size={14}
                              className="text-emerald-600"
                              strokeWidth={3}
                      ...
                      return (
                        <td
                          key={plan._id + feat._id}
                          className={`px-5 py-3.5 text-center ${plan.isPopular ? "bg-purple-50/10" : ""}`}
                        >
                          {cell}
                        </td>
                      );
                  ...
                  <tr className="bg-slate-50/50">
                    ...
                    {plans.map((plan) => (
                      <td key={plan._id} className="px-5 py-4 text-center">
                        <div
                          className={`mx-auto inline-block px-5 py-2 rounded-xl text-xs font-black uppercase tracking-wider ${
                            plan.isPopular
        ...
        <div className="fixed inset-0 bg-black/40 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          ...
          >
            ...
            <h3 className="text-xl font-black text-slate-800 mb-6 flex items-center gap-2">
              <div className="w-9 h-9 bg-blue-100 text-blue-600 flex items-center justify-center rounded-xl">
                <Settings2 size={18} />
              </div>
              Add Feature
            </h3>
            <div className="space-y-4 mb-6">
              <div>
                ...
                <input
                  ...
                  value={newFeat.name}
                  onChange={(e) =>
                    setNewFeat({ ...newFeat, name: e.target.value })
                  }
              ...
              <div>
                ...
                <select
                  value={newFeat.type}
                  onChange={(e) =>
                    setNewFeat({ ...newFeat, type: e.target.value })
                  }
            ...
            <button
              type="submit"
              className="w-full py-3 bg-blue-600 text-white rounded-xl font-bold shadow-lg shadow-blue-600/25 hover:bg-blue-700 transition-all active:scale-95"
            >
              Save Feature
            </button>
          </form>
        ...
        <div className="fixed inset-0 bg-black/40 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          ...
          >
            ...
            <div className="p-6 overflow-y-auto flex-1 space-y-6">
              ...
              <div>
                <h4 className="text-xs font-black uppercase text-slate-400 tracking-widest mb-3 flex items-center gap-2">
                  <div className="w-2 h-2 rounded-full bg-blue-500" /> Core
                  Details
                </h4>
                <div className="grid grid-cols-3 gap-4">
                  <div>
                    ...
                    <input
                      ...
                      className="w-full p-2.5 bg-slate-50 border border-slate-200 rounded-xl font-bold text-slate-800 focus:ring-2 focus:ring-blue-500 outline-none"
                  ...
                  <div>
                    ...
                    <input
                      value={form.qrCodes}
                      onChange={(e) =>
                        setForm({ ...form, qrCodes: e.target.value })
                      }
                      className="w-full p-2.5 bg-slate-50 border border-slate-200 rounded-xl font-bold text-slate-800 focus:ring-2 focus:ring-blue-500 outline-none"
                      placeholder="25,000"
                    />
                  ...
                  <div>
                    ...
                    <input
                      value={form.minQrPerOrder}
                      onChange={(e) =>
                        setForm({ ...form, minQrPerOrder: e.target.value })
                      }
                      className="w-full p-2.5 bg-slate-50 border border-slate-200 rounded-xl font-bold text-slate-800 focus:ring-2 focus:ring-blue-500 outline-none"
                      placeholder="1,000"
                    />
              ...
              <div>
                <h4 className="text-xs font-black uppercase text-slate-400 tracking-widest mb-3 flex items-center gap-2">
                  <div className="w-2 h-2 rounded-full bg-violet-500" /> Pricing
                  per Billing Cycle
                </h4>
                <div className="grid grid-cols-3 gap-3">
                  {CYCLE_KEYS.map((cyc) => {
                    ...
                    return (
                      ...
                      >
                        <div
                          className={`px-4 py-2.5 text-center font-black text-sm uppercase tracking-wider ${
                            colr === "amber"
                        ...
                        <div className="p-3 space-y-3">
                          ...
                          <div>
                            ...
                            <input
                              value={cp.validity ?? "-"}
                              onChange={(e) =>
                                setCF(cyc, "validity", e.target.value)
                              }
                              className="w-full p-2 bg-white border border-slate-200 rounded-lg font-bold text-slate-800 focus:ring-2 focus:ring-blue-500 outline-none text-sm"
                          ...
                          <div>
                            ...
                            <input
                              value={cp.saveText ?? "-"}
                              onChange={(e) =>
                                setCF(cyc, "saveText", e.target.value)
                              }
                              className="w-full p-2 bg-white border border-slate-200 rounded-lg font-bold text-slate-800 focus:ring-2 focus:ring-blue-500 outline-none text-sm"
              ...
              <div className="flex gap-4">
                <label
                  className={`flex-1 flex items-center gap-3 p-4 border-2 rounded-xl cursor-pointer transition-all select-none ${form.isPopular ? "bg-purple-50 border-purple-200" : "bg-slate-50 border-slate-200"}`}
                >
                  <input
                    type="checkbox"
                  ...
                  />
                  <div
                    className={`w-5 h-5 rounded flex items-center justify-center transition-colors ${form.isPopular ? "bg-purple-600 text-white" : "bg-slate-200 text-transparent"}`}
                  >
                    <Check size={14} strokeWidth={3} />
                  </div>
                  <span
                    className={`font-bold text-sm ${form.isPopular ? "text-purple-800" : "text-slate-600"}`}
                  >
                    Most Popular
                  </span>
                </label>
                <label
                  className={`flex-1 flex items-center gap-3 p-4 border-2 rounded-xl cursor-pointer transition-all select-none ${form.isTrial ? "bg-amber-50 border-amber-200" : "bg-slate-50 border-slate-200"}`}
                >
                  <input
                    type="checkbox"
                    checked={form.isTrial}
                    onChange={(e) =>
                      setForm({ ...form, isTrial: e.target.checked })
                    }
                    className="hidden"
                  />
                  <div
                    className={`w-5 h-5 rounded flex items-center justify-center transition-colors ${form.isTrial ? "bg-amber-500 text-white" : "bg-slate-200 text-transparent"}`}
                  >
                    <Check size={14} strokeWidth={3} />
                  </div>
                  <span
                    className={`font-bold text-sm ${form.isTrial ? "text-amber-800" : "text-slate-600"}`}
                  >
                    Trial Plan
                  </span>
                </label>
                      ...
                      >
                        ...
                        {f.type === "boolean" ? (
                          <select
                            value={String(cur)}
                            onChange={(e) =>
                              setFV(f._id, e.target.value === "true")
                            }
                          ...
                          <input
                            value={cur}
                            onChange={(e) => setFV(f._id, e.target.value)}
                            className="border border-slate-200 rounded-lg px-3 py-1.5 text-sm w-40 font-bold bg-white focus:ring-2 focus:ring-blue-500 outline-none"
                            placeholder="Value"
                          />
                        )}
                      </div>
            ...
            <div className="p-6 border-t border-slate-100 flex justify-end gap-3 rounded-b-2xl bg-white">
              ...
              <button
                type="submit"
                className="px-6 py-2.5 bg-blue-600 text-white rounded-xl font-bold shadow-lg shadow-blue-600/25 hover:bg-blue-700 transition-all active:scale-95 flex items-center gap-2"
              >
        ...
        <div className="fixed inset-0 bg-black/40 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md p-7 relative">
            ...
            <h3 className="text-xl font-black text-slate-800 mb-2 flex items-center gap-2">
              <div className="w-9 h-9 bg-violet-100 text-violet-600 flex items-center justify-center rounded-xl">
                <Sliders size={18} />
              </div>
              Billing Cycle Labels
            </h3>
            <p className="text-slate-500 text-sm mb-6 font-medium">
              Customize the tab labels shown to users.
            </p>
            <div className="space-y-4">
              {[
                ...
                <div key={c.key}>
                  ...
                  <input
                    value={configForm[c.key] || ""}
                    onChange={(e) =>
                      setConfigForm({ ...configForm, [c.key]: e.target.value })
                    }
            ...
            >
              {configSaving ? (
                ...
                <>
                  <Check size={16} strokeWidth={3} /> Save Labels
                </>
              )}
            </button>
          </div>
