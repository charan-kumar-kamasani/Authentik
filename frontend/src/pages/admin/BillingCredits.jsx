...
const BillingCredits = () => {
  ...
  const handleBuyPlan = async (plan) => {
    ...
    const credits =
      parseInt(String(plan.qrCodes || "0").replace(/,/g, ""), 10) || 0;
  ...
  const handleApplyCoupon = async () => {
    ...
    const baseAmt = selectedPlan
      ? parseInt(String(selectedPlan.qrCodes || "0").replace(/,/g, ""), 10) *
        (selectedPlan.pricePerQr || 0)
      : parseInt(topupQty) * 5;
    try {
          ...
          <div className="relative z-10 flex items-start gap-4">
            ...
            <div className="flex-1">
              ...
              <p className="text-sm text-purple-800 leading-relaxed">
                This account is configured for payment gateway testing. When you
                initiate any payment, you will be charged{" "}
                <span className="font-black">₹{testAmount}</span> instead of the
                actual package price. This allows you to test the complete
                payment flow without paying the full amount.
              </p>
              <div className="mt-3 flex items-center gap-2 text-xs text-purple-700">
                <AlertCircle size={14} strokeWidth={2.5} />
                <span className="font-semibold">
                  All credits will be added normally after successful payment
                </span>
              </div>
            </div>
          </div>
      ...
      <div className="bg-white rounded-2xl border border-slate-200/80 shadow-sm overflow-hidden">
        <div className="px-6 py-4 border-b border-slate-100 flex items-center justify-between">
          ...
          <button
            ...
            className="text-xs font-bold text-blue-600 hover:text-blue-700 flex items-center gap-1.5 transition-colors px-3 py-1.5 bg-blue-50 rounded-lg hover:bg-blue-100"
          >
                    ...
                    >
                      <td className="px-6 py-4">
                        <div className="flex items-center gap-2.5">
                          <div
                            className={`w-8 h-8 rounded-lg flex items-center justify-center bg-${cfg.color}-100 text-${cfg.color}-600`}
                          >
                            <Icon size={14} />
                          </div>
                      ...
                      <td className="px-6 py-4">
                        <div className="text-xs text-slate-500 font-medium">
                          <div className="flex items-center gap-1.5">
                            <Calendar size={12} /> {fmt(txn.createdAt)}
                          </div>
        ...
        {transactions.length > 5 && (
          <div className="px-6 py-4 bg-slate-50/50 border-t border-slate-100">
            ...
            <p className="text-xs text-slate-500 text-center mt-3">
              View complete transaction history with payment details, status
              tracking, and invoice downloads
            </p>
          </div>
        )}
      </div>
      ...
      {showBuyModal && (
        ...
        >
          <div
            ...
            className={`bg-white rounded-2xl shadow-2xl w-full overflow-hidden max-h-[90vh] flex flex-col ${buyView === "comparison" ? "max-w-6xl" : "max-w-lg"}`}
          >
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h3 className="text-lg font-black text-white">
                    {buyView === "checkout" ? "Checkout" : "Buy Credits"}
                  </h3>
                  <p className="text-violet-100 text-xs font-medium">
                    {buyView === "checkout"
                      ? "Review & pay"
                      : "Add QR credits to your balance"}
                  </p>
                </div>
              </div>
              {!processing && (
            ...
            <div className="p-6 overflow-y-auto flex-1">
              ...
              <div className="bg-violet-50 border border-violet-200 rounded-xl p-3 mb-5 flex items-center justify-between">
                ...
                <span className="text-lg font-black text-violet-700">
                  {(balance?.qrCredits || 0).toLocaleString()} credits
                </span>
              </div>
                ...
                <div className="space-y-3">
                  ...
                  >
                    ...
                    <div className="flex-1">
                      ...
                      <div className="text-xs text-slate-500 font-medium mt-0.5">
                        Compare all plans with full pricing breakdown
                      </div>
                    </div>
                  ...
                  >
                    ...
                    <div className="flex-1">
                      ...
                      <div className="text-xs text-slate-500 font-medium mt-0.5">
                        Choose from pricing plans with bulk discounts
                      </div>
                    </div>
                    <ChevronRight size={16} className="text-slate-400" />
                  </button>
                  ...
                  >
                    ...
                    <div className="flex-1">
                      ...
                      <div className="text-xs text-slate-500 font-medium mt-0.5">
                        Buy exact quantity at ₹5 per QR credit
                      </div>
                    </div>
                    <ChevronRight size={16} className="text-slate-400" />
                  </button>
                        ...
                        <table className="w-full text-sm min-w-[900px]">
                          <thead>
                            <tr className="bg-slate-50">
                              <th className="px-4 py-3 text-left text-xs font-black text-slate-600 uppercase tracking-wider border-r border-slate-200 w-48">
                                Feature
                              </th>
                              {plans.map((plan) => {
                                const isOnDemand =
                                ...
                                const credits = isOnDemand
                                  ? 0
                                  : parseInt(
                                      String(plan.qrCodes || "0").replace(
                                        /,/g,
                                        "",
                                      ),
                                      10,
                                    );
                                const calculatedPrice = isOnDemand
                                  ? 0
                                  : credits * (plan.pricePerQr || 0);
                                return (
                                  <th
                                    key={plan._id}
                                  ...
                                  >
                                    ...
                                    {plan.isPopular && (
                                      <div className="mt-2">
                                        <span className="inline-block text-[9px] font-bold bg-amber-100 text-amber-700 px-2 py-0.5 rounded-md">
                                          POPULAR
                                        </span>
                                      </div>
                                    )}
                          ...
                          <tbody className="divide-y divide-slate-200">
                            ...
                            <tr className="hover:bg-slate-50">
                              <td className="px-4 py-3 text-xs font-semibold text-slate-600 border-r border-slate-200">
                                <span className="flex items-center gap-1.5">
                                  <Calendar size={14} /> Validity (Yearly)
                                </span>
                              </td>
                              {plans.map((plan) => (
                                <td
                                  key={plan._id}
                                  className="px-4 py-3 text-center font-semibold text-slate-700 border-r border-slate-200 last:border-r-0"
                                >
                            ...
                            <tr className="hover:bg-slate-50 bg-amber-50/30">
                              <td className="px-4 py-3 text-xs font-bold text-orange-600 border-r border-slate-200">
                                Save (Yearly)
                              </td>
                              {plans.map((plan) => (
                                <td
                                  key={plan._id}
                                  className="px-4 py-3 text-center font-bold text-orange-600 border-r border-slate-200 last:border-r-0"
                                >
                                  {plan.saveText || "-"}
                                </td>
                              ))}
                            ...
                            <tr className="hover:bg-slate-50">
                              ...
                              {plans.map((plan) => {
                                ...
                                return (
                                  <td
                                    key={plan._id}
                                    className="px-4 py-3 text-center font-bold text-slate-800 border-r border-slate-200 last:border-r-0"
                                  >
                                    {isOnDemand
                                      ? "On-Demand"
                                      : plan.qrCodes === "Unlimited"
                                        ? plan.qrCodes
                            ...
                            <tr className="hover:bg-slate-50 bg-blue-50/50">
                              <td className="px-4 py-3 text-xs font-black text-slate-800 border-r border-slate-200">
                                Total Price
                              </td>
                              {plans.map((plan) => {
                                const isOnDemand =
                    ...
                    <div className="max-h-[320px] overflow-y-auto space-y-2 pr-1">
                      {plans.map((plan) => {
                        ...
                        const credits = isOnDemand
                          ? 0
                          : parseInt(
                              String(plan.qrCodes || "0").replace(/,/g, ""),
                              10,
                            ) || 0;
                  ...
                  {topupQty && parseInt(topupQty) > 0 && (
                    <div className="bg-emerald-50 border border-emerald-200 rounded-xl p-3 flex items-center justify-between">
                      <span className="text-xs font-bold text-emerald-700">
                        {parseInt(topupQty).toLocaleString()} QRs × ₹5
                      </span>
                      <span className="text-lg font-black text-emerald-700">
                        ₹{(parseInt(topupQty) * 5).toLocaleString()}
                      </span>
                    </div>
                  )}
                  <button
              ...
              {!processing && buyView === "checkout" && (
                <div className="space-y-4">
                  <button
                    onClick={() => {
                      setBuyView(selectedPlan ? "plans" : "topup");
                      setPriceBreakdown(null);
                      setCouponApplied(null);
                      setCouponCode("");
                    }}
                    ...
                    {selectedPlan ? (
                        ...
                        <div className="flex items-center justify-between mb-1">
                          <div>
                            ...
                            <div className="text-xs text-slate-500">
                              {(
                                parseInt(
                                  String(selectedPlan.qrCodes || "0").replace(
                                    /,/g,
                                    "",
                                  ),
                                ) || 0
                              ).toLocaleString()}{" "}
                              credits × ₹{selectedPlan.pricePerQr}/QR
                            </div>
                          </div>
                      ...
                      <div className="flex items-center justify-between">
                        <div>
                          ...
                          <div className="text-xs text-slate-500">
                            {parseInt(topupQty).toLocaleString()} credits × ₹5
                          </div>
                        </div>
                        <div className="font-black text-slate-800">
                          ₹{(parseInt(topupQty) * 5).toLocaleString()}
                              ...
                              <div className="flex items-start gap-2">
                                ...
                                <div>
                                  ...
                                  <p className="text-xs text-purple-700 leading-relaxed">
                                    ...
                                    for testing purposes. Full credits will be
