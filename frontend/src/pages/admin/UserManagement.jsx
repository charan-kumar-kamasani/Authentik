  ...
  const handleEditBrand = async (brand) => {
    ...
    if (companyId && (role === 'admin' || role === 'superadmin')) {
      try {
        ...
        const mapEmail = (val) => ({ 
          value: typeof val === 'string' ? val : (val?.email || val?.value || ""), 
          password: "", 
          otpSent: true, 
          verified: true, 
          sending: false, 
        ...
        if (Array.isArray(company.officialEmails) && company.officialEmails.length > 0) {
          setOfficialEmails(company.officialEmails.map(mapEmail));
        }
        if (Array.isArray(company.authorizerEmails) && company.authorizerEmails.length > 0) {
          setAuthorizerEmails(company.authorizerEmails.map(mapEmail));
        }
        if (Array.isArray(company.creatorEmails) && company.creatorEmails.length > 0) {
          setCreatorEmails(company.creatorEmails.map(mapEmail));
        }
  ...
  const handleEditCompany = async (company) => {
    ...
    try {
      ...
      if (bData && Array.isArray(bData) && bData.length > 0) {
        ...
        setCompanyBrands([{ brandName: "", brandLogo: "", logoType: "url", status: 'active' }]);
      }
    } catch (e) {
                             ...
                             {b._id ? (
                               <button 
                                 ...
                                 onClick={() => {
                                   ...
                                   updateCompanyBrandRow(idx, 'status', newStatus);
                                 }}
                                 className={`mb-1 px-4 py-2.5 rounded-xl text-xs font-black transition-all uppercase tracking-wider shadow-sm flex items-center gap-2
                                   ${b.status === 'blocked' 
                                     ? 'bg-amber-100 text-amber-700 hover:bg-amber-200' 
                                     : 'bg-emerald-100 text-emerald-700 hover:bg-emerald-200'}`}
                                 title={b.status === 'blocked' ? 'Activate Brand' : 'Pause Brand'}
                               >
                                 {b.status === 'blocked' ? (
                                   <><Play size={14} fill="currentColor" /> Unpause</>
                                 ) : (
          ...
          <div className="flex items-center gap-3 animate-in slide-in-from-right duration-700">
            ...
            <button 
              onClick={() => {
                ...
                setEditingCompany(null);
                setEditingBrand(null);
                setActiveTab(role === 'superadmin' ? 'create' : 'createStaff');
                // Optional: reset forms here if needed
              }}
      ...
      {showPassword && (
        <div className="relative group">
          <input 
            ...
            placeholder={entry.value ? "New Pwd (Optional)" : "Password"} 
