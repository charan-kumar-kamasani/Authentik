...
export default function AnimatedCTA({ onClick, className = "" }) {
  ...
  return (
    <button
      onClick={onClick}
      className={`group relative h-[64px] overflow-hidden rounded-full font-[900] tracking-widest transition-all shadow-lg hover:shadow-xl hover:scale-[1.02] active:scale-95 text-lg flex items-center justify-center gap-4 ${className}`}
    >
      ...
      <span className="relative z-10 flex items-center gap-3 text-white px-8">
        <span className="relative h-7 w-56">
