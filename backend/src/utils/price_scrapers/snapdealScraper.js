...
const getSnapdealData = async (url) => {
    ...
    try {
      ...
      page.on('response', async (response) => {
        try {
          ...
          if (reqUrl.includes('/ac/get') || reqUrl.includes('api') || reqUrl.includes('json')) {
            ...
            if (contentType && contentType.includes('application/json')) {
              ...
              const findData = (obj) => {
                ...
                if (obj.sellingPrice || obj.mrp) {
                   if (!result.price) result.price = cleanPrice(obj.sellingPrice || obj.price);
      ...
      // Fallback DOM for Snapdeal
      if (!result.price) {
          const html = await page.content();
          const cheerio = require('cheerio');
          const $ = cheerio.load(html);
          result.price = cleanPrice($('.payBlkBig').text());
          result.mrp = cleanPrice($('.pdp-final-price .strike').text());
      }
      
      setTimeout(() => resolveData(result), 2000);
