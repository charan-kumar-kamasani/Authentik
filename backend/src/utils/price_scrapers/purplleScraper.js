...
const getPurplleData = async (url) => {
          ...
          if (reqUrl.includes('/api/') || reqUrl.includes('graphql') || reqUrl.includes('product')) {
            ...
            if (contentType && contentType.includes('application/json')) {
              ...
              const findData = (obj) => {
                ...
                if (obj.offer_price || obj.mrp || obj.price || obj.selling_price) {
                   if (!result.price) result.price = cleanPrice(obj.offer_price || obj.selling_price || obj.price);
                   if (!result.mrp) result.mrp = cleanPrice(obj.mrp);
                   found = true;
                }
                if (obj.image_url && !result.siteImage) result.siteImage = obj.image_url;
                if (obj.rating && !result.rating) result.rating = parseFloat(obj.rating);
                if (obj.rating_count && !result.reviewsCount) result.reviewsCount = obj.rating_count.toString();
