...
router.post('/users/staff', protect, async (req, res) => {
    ...
    const { name, email, password, role, brandId: bodyBrandId, companyId: bodyCompanyId } = req.body; 
        ...
        if (req.user.role === 'company') {
            ...
            if (bodyBrandId) brandId = bodyBrandId;
            if (bodyCompanyId) companyId = bodyCompanyId;
            // If brandId provided but no companyId, try to resolve it from Brand
    ...
    try {
      console.log("Get Staff Users - Requester:", req.user.email, "Role:", req.user.role);
            ...
            if (req.user.role === 'superadmin') {
                if (brandId) {
                    const staff = await User.find({ brandId, role: { $in: ['authorizer', 'creator'] } });
